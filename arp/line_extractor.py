...
class LineExtractor(Config):
    ...
    def get_detection_line(self, im, boxes, segms=None, keypoints=None, thresh=0.9, kp_thresh=2,
        ...
        if img_debug:
            ...
            perspective_img = np.zeros((self.IMAGE_HEI, self.IMAGE_WID,3), np.uint8)
        ...
        for i in sorted_inds:
            ...
            class_str = self.get_class_string(classes[i], score, dataset)
            # show class (off by default)
            if show_class and img_debug:
                im = self.vis_class(im, (bbox[2], bbox[3] - 2), class_str)
            ...
            if segms is not None and len(segms) > i:
                ...
                if img_debug and (type in line_class):
                    # mid_im = np.zeros(im.shape, np.uint8)
                    mid_im = self.vis_roi(mid_im, masks[..., i], color_mask)
                    # mid_im = self.vis_roi(mid_im, masks[..., i], np.array([255.,255.,255.]))
                    # cv2.imwrite("/home/xhpan/Documents/detect_area/{}-{}.png".format(frame_id, i), mid_im)
                if img_debug:
                    perspective_color = dummy_datasets.get_perspective_color(color_index)
                    im, perspective_img = self.vis_mask(im, perspective_img, curve_objs, masks[..., i], color_mask, perspective_color, type, score)
                    color_index += 1
                elif type in line_class:
                    t_find_curve = time.time()
                    self.build_curve_objs(curve_objs, masks[..., i], type, score, img_debug)
    ...
    def optimize_parabola(self, perspective_img, curve_objs, img_debug, frame_id):
        ...
        for curve_obj in curve_objs:
            ...
            if curve_type != lane_line.BOUNDARY or curve_type == lane_line.FORK_LINE:
                ...
                if offset_x > self.lane_wid/2 and float(offset_y) / offset_x < self.BOX_SLOPE_LIMITED:
                    continue
        ...
        if is_fork_enable and is_fork:
            ...
            for index in range(len(parabola_param_np)):
                if classes_param[index] != lane_line.BOUNDARY:
                    continue
                if parabola_param_np[index][-2] < self.fork_pos[0]:
                    left.append(parabola_param_np[index])
                else:
                    right.append(parabola_param_np[index])
            left_center = self.fork_pos[0] - self.lane_wid/2
            right_center = self.fork_pos[0] + self.lane_wid/2
            ...
            left_fork_index = parabola_param_np[:, -2] <= self.fork_pos[0]
            right_fork_index = parabola_param_np[:, -2] > self.fork_pos[0]
        ...
        for parabola_param_np, classes_param, parabola_box, boundary in zip(parabola_param_list, classes_param_list, parabola_box_list, boundarys):
            ...
            for tmp_box in parabola_box:
                cv2.rectangle(perspective_img, (int(tmp_box[0] + self.IMAGE_WID/2), int(tmp_box[1] + self.IMAGE_HEI)),
                              (int(tmp_box[2] + self.IMAGE_WID/2), int(tmp_box[3] + self.IMAGE_HEI)), (100, 0, 0), 2)

            ret = self.get_good_parabola(parabola_param_np, parabola_box)
            if ret is None:
                print ("errer: bad frame detection, didn't find good parabola!")
                continue
            good_parabola, index_param = ret
            curve = np.arange(-self.IMAGE_HEI, 0, 10)
            x_log = []
            for index, parabola in enumerate(parabola_param_np):
                if index == index_param:
                    ...
                    if img_debug:
                        ...
                        color = (100, 0, 20)
                        cv2.polylines(perspective_img, np.int32([np.vstack((y + self.IMAGE_WID/2, curve + self.IMAGE_HEI)).T]), False, color, thickness=2)
                else:
                    # predict_parabola = parabola[0:3]
                    y = self.get_parabola_y(good_parabola, parabola[-1])
                    relative_point = (parabola[-1], y)
                    relative_predict = (parabola[-1], self.get_parabola_y(parabola[0:3], parabola[-1]))
                    predict_parabola = math_utils.get_parabola_by_distance(good_parabola, parabola[5] - y)
                    ...
                    if parabola_param_np[index][0] == 0:
                        pass
                    if img_debug:
                        # predict_parabola = parabola_param_np[index][0:3]
                        color = (255, 255, 255)
                        x_log.append(predict_parabola[2])
                        y = predict_parabola[0] * curve * curve + predict_parabola[1] * curve + predict_parabola[2]
                        cv2.polylines(perspective_img, np.int32([np.vstack((y + self.IMAGE_WID/2, curve + self.IMAGE_HEI)).T]), False, color, thickness=2)
                        cv2.rectangle(perspective_img, (int(relative_point[1] + self.IMAGE_WID/2 - 5), int(relative_point[0] + self.IMAGE_HEI - 5)),
                                      (int(relative_point[1] + self.IMAGE_WID/2 + 5), int(relative_point[0] + self.IMAGE_HEI + 5)), (255, 0, 0), 2)
                        cv2.rectangle(perspective_img, (int(relative_predict[1] + self.IMAGE_WID/2 - 5), int(relative_predict[0] + self.IMAGE_HEI - 5)),
                                      (int(relative_predict[1] + self.IMAGE_WID/2 + 5), int(relative_predict[0] + self.IMAGE_HEI + 5)), color, 2)
            ret_parabola.append([parabola_param_np, classes_param])
        if len(ret_parabola) == 0:
            return None
        if img_debug and is_fork and not self.fork_pos is None:
            triangle_size = 10
            triangle_center = [self.fork_pos[0] + self.IMAGE_WID/2, self.fork_pos[1] + self.IMAGE_HEI]
    ...
    def vis_mask(self, img, perspective_img, curve_objs,  mask, col, perspective_color, classs_type, score, alpha=0.4, show_border=True, border_thick=1):
        ...
        if classs_type in line_class:
            ...
            mask, top_idx = self.build_curve_objs(curve_objs, mask, classs_type, score, True)
    ...
    def get_class(self, class_index, dataset):
        class_text = dataset.classes[class_index] if dataset is not None else \
            'id{:d}'.format(class_index)
        return class_text

    def get_closest_points(self, points, y):
        index_close = -1
        max_distance = 1000
    ...
    def add2curve(self, curve_objs, point, type, score):
        matched = False
        for i in range(len(curve_objs)):
            ...
            if p1[0] != p2[0] and abs(p1[0] - p2[0] > self.IMAGE_HEI / 6):
            ...
            if distanceY1 < 0 and distanceY2 > 0:
                if distanceX < 10:
                    ...
                    continue
            if abs(distanceY1) > self.IMAGE_HEI / 3 and abs(distanceY2) > self.IMAGE_HEI / 3:
                # print ("distanceY1:{}, distanceY2:{}, reference:{}".format(distanceY1, distanceY2, self.IMAGE_HEI / 3))
                continue
        ...
        if matched:
            if const.CLUSTER_DEBUG:
                debug_im = np.zeros((self.IMAGE_HEI, self.IMAGE_WID, 3), np.uint8)
                for _, pts in enumerate(curve_objs):
                    debug_points = pts["points"]
                    for p in debug_points:
                        cv2.circle(debug_im, (int(p[0] + self.IMAGE_WID / 2), int(p[1] + self.IMAGE_HEI)), 5, tuple(const.DEBUG_CORLOR[_]),
                                   thickness=5)
        ...
        if const.CLUSTER_DEBUG:
            ...
            for _, pts in enumerate(curve_objs):
                debug_points = pts["points"]
                for p in debug_points:
                    cv2.circle(debug_im, (int(p[0] + self.IMAGE_WID / 2), int(p[1] + self.IMAGE_HEI)), 5, tuple(const.DEBUG_CORLOR[_]), thickness=5)
            cv2.imwrite("/media/administrator/deeplearning/detectron/debug/{}.png".format(int(round(time.time() * 1000))), debug_im)

    def build_curve_objs(self, curve_objs, mask, classs_type, score, img_debug):
        line_class = dummy_datasets.get_line_dataset()
        top_idx = None
        if classs_type in line_class:
            # mask = cv2.undistort(mask, self.camera_mtx, dist, None)
            ...
            top_idx = cv2.perspectiveTransform(np.array([points], dtype=np.float32), np.array(self.H))

            top_idx = top_idx[0].astype(np.int32)

            # top_idx = top_idx[::10]
            top_idx = top_idx[(top_idx[:,0] > 0) & (top_idx[:,0] < self.IMAGE_WID) & (top_idx[:,1] > 0) & (top_idx[:,1] < self.IMAGE_HEI)]
            ...
            if len(top_idx) > 10:
                ...
                for single_point in top_idx:
                    if single_point[1] != y_start:
                        self.add2curve(curve_objs, [(x_end + x_start) / 2 -self.IMAGE_WID /2, y_start - self.IMAGE_HEI, x_start -self.IMAGE_WID /2,
                                                    x_end -self.IMAGE_WID /2], classs_type, score)
                        y_start = single_point[1]
                        x_start = single_point[0]
                        x_end = x_start
                    else:
                        if single_point[0] - x_end > self.lane_wid / 10:
                            self.add2curve(curve_objs, [(x_end + x_start)/2 -self.IMAGE_WID /2, y_start - self.IMAGE_HEI, x_start -self.IMAGE_WID /2,
                                                        x_end -self.IMAGE_WID /2], classs_type, score)
                            y_start = single_point[1]
                            x_start = single_point[0]
    ...
    def vis_roi(self, img,  mask, col):
        """Visualizes the class."""
        img = img.astype(np.float32)
        idx = np.nonzero(mask)#(1208,1920
        img[idx[0], idx[1], :] *= 1.0 - 0.4
        img[idx[0], idx[1], :] += 0.9 * col
        return img.astype(np.uint8)

    def vis_perspective(self, img):
        """Visualizes the class."""
        img = img.astype(np.float32)
        top_im = cv2.warpPerspective(img, self.H, (self.IMAGE_WID, self.IMAGE_HEI))
    ...
    def get_good_parabola(self, coefficient, parabola_box):
        good_index = 0
        min_gradient = 1
        perfect_avg_indexs = []
        good_avg_indexs = []
        ...
        for box_index, box in enumerate(parabola_box):
            ...
            if (hei > 0.8*self.IMAGE_HEI and wid < 25) or (hei > 0.5*self.IMAGE_HEI and wid < 15):
                perfect_avg_indexs.append(box_index)
            elif hei > 0.6*self.IMAGE_HEI and const.CAMERA_TYPE == 2:#600/60
                good_avg_indexs.append(box_index)
            elif hei > self.IMAGE_HEI / 2 and hei / (box[2] - box[0]) > 10:#600/60
                good_min_indexs.append(box_index)
        if len(perfect_avg_indexs) > 0:
            good_index = self.get_good_index(coefficient, perfect_avg_indexs)
        elif len(good_avg_indexs) > 0:
            good_index = self.get_good_index(coefficient, good_avg_indexs)
            ...
            for index in good_box:
                ...
                gradient1 = self.get_gradient(param, -self.IMAGE_HEI)
                gradient2 = self.get_gradient(param, 0)

                hei = box[3] - box[1]
                wid = box[2] - box[0]
                if abs(gradient2) > (1./self.PARABORA_SLOPE_LIMITED) and hei < self.IMAGE_HEI / 3:
                    continue
