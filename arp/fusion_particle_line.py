...
class Particle(object):
    ...
    def move_high_freq(self, speed, w, noisy=True):
        ...
        self.x_move += self.r * np.sin(self.h + dalta_time * w) - self.r * np.sin(self.h)
        self.y_move +=-self.r * np.cos(self.h + dalta_time * w) + self.r * np.cos(self.h)
        self.h += dalta_time * w
        self.move_time = time.time()
...
class CarWayPoint(Particle):
    ...
    def info(self):
        return self.h, self.x_move, self.y_move, self.r
...
class FusionParticle(threading.Thread):
    def __init__(self, x_right_line, queue):
        ...
        self.particles = Particle.create_particle_random(x_right_line, 0, np.pi/2, PARTICLE_COUNT)
    ...
    def update(self, x_right_line, param):
        ...
        h, x_move, y_move, R = self.way_point.info()
        print("tmp1 x_move1:{}, param:{}".format(x_move, str(param)))
        dalta_h = h - np.pi / 2
        angle = math.atan2(param[1], 1)#y=2ax^2 + b
        map_origin = abs(R * (1-math.cos(dalta_h)))
        map_dest = abs(R * (math.cos(angle - dalta_h) - math.cos(angle)))
        x_move = x_move * (map_dest / map_origin)
        print("tmp1 x_move2:{}, map_origin:{}, map_dest:{}".format(x_move, map_origin, map_dest))
        ...
        if x - x_right_line > LANE_WID / 2:
            x -= LANE_WID
        elif x - x_right_line < -LANE_WID / 2:
            x += LANE_WID
        x_estimate = x
        x_change = (x - x_move)
        # ---------- Show current state ----------
        print ("current state, x_right_line:{}, x:{}".format(x_right_line, x))
