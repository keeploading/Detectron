...
def add_shape_points_outputs(model, blob_in, dim):
...
def add_shape_points_losses(model):
    ...
    kps_prob, loss_kps = model.net.SoftmaxWithLoss(
        ['kps_score_reshaped', 'shape_points_locations_int32', 'shape_points_weights'],
        ['kps_prob', 'loss_kps'],
    ...
    if not cfg.KRCNN.NORMALIZE_BY_VISIBLE_KEYPOINTS:
        ...
        # of the N keypoints.)
        model.StopGradient(
            'shape_points_loss_normalizer', 'shape_points_loss_normalizer'
        )
        loss_kps = model.net.Mul(
            ['loss_kps', 'shape_points_loss_normalizer'], 'loss_kps_normalized'
        )
